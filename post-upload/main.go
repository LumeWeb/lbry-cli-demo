...
func main() {
	...
	logger := log.New(os.Stdout, "[POST-UPLOAD] ", log.LstdFlags)

	zapLogger.Info("Starting post-upload demo",
	...
	// Generate 10MB random data blob
	...
	// Upload the stream
	logger.Printf("Uploading stream...")
	...
	uploadResp, err := client.UploadStream(file, uploadFile)
	if err != nil {
		logger.Fatalf("Failed to upload stream: %v", err)
	}

	logger.Printf("Stream uploaded successfully. Upload hash: %s", uploadResp.UploadHash)
